...
**`simplify` ist kein Subagent, sondern die built-in `/simplify`-Skill.** Ein opt-in Roster-Glied (`KNOWN_AGENTS`, **nicht** `DEFAULT_AGENTS`) ohne `.md`-Template — der Master ruft die Claude-Code-CLI-Skill `/simplify` direkt über das **`Skill`-Tool** auf, statt einen Task-Subagent zu spawnen. Position: **nach** der finalen Tester-Verifikation und **vor** dem reviewer (`developer → tester(grün) → /simplify → tester(re-verify) → reviewer`), damit der reviewer den bereits vereinfachten Diff liest. Die Skill editiert den Worktree selbst (reuse/simplification/efficiency/altitude) — das ist die **eine** sanktionierte Ausnahme zum „Edit files yourself: NEVER" (Tool-Call, kein Hand-Editing); `build_orchestrator_prompt` hält sie aus der Task-Subagent-Team-Zeile raus und entschärft die NEVER-Regel nur, wenn `simplify` im Roster steht. Freigeschaltet wird sie gescopt als `Skill(simplify)` in `_augment_tools_for_multi_agent` — **nur** bei aktivem `simplify`, nie das offene `Skill` (Allowlist bleibt die enge Verteidigungslinie). Bewusst opt-in, weil `/simplify` intern **4 parallele Review-Agenten** startet (Kostentreiber). Mantra 3 intakt: forge taktet den Skill-Aufruf an, entscheidet nichts daran; kein Schema-Bump (`agents_used` darf `simplify` als „mitgewirkt" tragen). **Live-Caveat:** ob die Skill im headless `claude -p`-Mode wirklich aufrufbar ist, ist nicht offiziell dokumentiert — gegen echtes `claude` verifizieren (Stream-Log `propose-*.jsonl` muss einen `Skill(simplify)`-Tool-Call zeigen).
