...
- [x] `forge run --focus / --prompt / --trigger / --max-iterations / --create-pr / --dry-run`
- [x] `forge analyze` — vier Standard-Reports als Markdown nach stdout oder `--output FILE`
- [x] `forge doctor` — Spec, Tools, claude/gh CLI, ANTHROPIC_API_KEY, forbidden-Sanity
- [x] `forge replay <run_id>` — Markdown-Timeline mit aufgelösten Prompt+Diff-Artefakten
- [x] typer-basiert + Rich-Tables, `forge`-Binary via `[project.scripts]`
- [x] Auto-Discovery der Spec via `find_repo_root` aufwärts ab CWD
- [x] Built-in Prompt-Templates (`legacy_test_revival`, `lint_cleanup`, `type_errors_reduction`)
- [x] Tests: 10 CLI-Tests inkl. End-to-End run → analyze → replay
- [ ] `forge sync-claude-md` (M2 — siehe CLAUDE.md-Lifecycle-Sektion)
- [ ] `forge init` (M2 — Template-Spec aus erkanntem Stack)

### 4b. `forge-adapters/github/`

- [x] Action-Templates: `forge-issue-trigger.yml`, `forge-ci-autofix.yml`,
  `forge-nightly.yml`, `forge-pr-merged.yml`
- [x] `gh pr create`-Wrapper mit strukturiertem Body (Files, Score-Delta, Diff-Excerpt)
- [x] `push_branch`, `render_pr_body`, `_extract_pr_number` Helpers
- [x] Webhook-Wrapper `record_pr_merged` / `record_pr_reverted`
- [x] `find_run_id_for_pr` — Verknüpfung Webhook → ursprünglicher Run
- [x] Issue-Bodies werden als UNTRUSTED USER CONTENT gewrappt (Spec Teil 7.3)
- [x] CLI-Integration: `forge run --create-pr` kettet Run + PR-Erzeugung
- [x] Tests: 16 Tests inkl. PR-Body-Rendering, YAML-Validität, Webhook-Roundtrip

**Erfolgskriterium Schritt 4 erreicht:** `forge run --create-pr --dry-run` läuft
End-to-End. CLI orchestriert WorktreeManager + SequentialRunner + GitHub-Adapter
sauber. Templates liegen bereit zum Kopieren in `.github/workflows/`. Volle
Pipeline ohne Claude-Aufruf testbar.
...
forges **eigene** CLAUDE.md (Pair-Programming-Doku):

- [x] `CLAUDE.md` am Repo-Root mit Architektur, Konventionen, Stolperfallen
- [x] Erwähnt explizit Prinzip 3 — forge ändert ihre eigene Konfiguration nie

CLAUDE.md-Lifecycle für **Target-Projekte** (separate Mechanik, M2):

- [ ] CLAUDE.md im Default-`forbidden`-Set der Reference-Spec
